...
Expanding our knowledge of functionality of tumor infiltrating immune cells requires first understanding blood and healthy tissue.
...
In addition, it is established that when immune cells transition into tissues, they are subjected to different stimuli that cause shifts in gene expression \citep{Fan2016}. 
...
Therefore, study of tumor infiltrating immune cells also requires characterization of corresponding healthy tissue and blood.

These requirements lead to complex experimental designs with multiple controls. 
...
To verify that observed cell states result from biological variance and not technical changes induced by the experimental protocol, it is critical to measure each sample multiple times to identify what fraction of observed variability is technical. 

Consequently, we expected to require approximately 1000 cells per replicate to properly profile each sample. 
...
The next section will how droplet-based sequencing makes this possible.

\section{Technical Characteristics of Droplet-based Sequencing} \label{technical-problems}

% todo fix tenses talk about how droplet-based sequencing works. 
Droplet-based sequencing allows thousands of cells to be sequenced using the same number of sequencing reads that were used to sequence 96 cells using plates, or 3-5 bulk samples. 
...
This allows the output of a sequencing experiment to be transformed from read abundances into a "count" matrix populated by molecules, and confers an added benefit of collapsing PCR outliers that amplify well into single observations, yielding more accurate counts \citep{Gruen2016}. 
...
This reduced cellular "jackpotting", where one read or gene accrues additional copies relative to its cellular abundance due to favorable amplification, which lead to more uniform sequencing coverage across cells. 
Together, these advancements reduced the per-cell cost to approximately \$0.4. 

% as a result, talk about why we selected InDrop (and then segue into how we intend to use it)
However, these improvements did not fully offset the decrease in sequencing depth.
Instead of spending 250,000 to 4 million or more reads per cell, droplet technologies measure phenotypes using $\frac{1}{40}th$ of the sequencing reads, as few as 20-50,000 per cell \citep{Klein2015,Macosko2015}.
...
Combined with the reduced numbers of reads associated with each cell, this leads to a phenomenon called "drop out" wherein the read-out for a cell may fail to capture all of the molecules of a given gene, causing it to incorrectly masquerade as unexpressed. 
...
Thus, many of these genes have low capture rates, and may drop out, eliminating historically effective mechanisms of identifying cell types. 
This phenomena is also computationally damaging, as the random drop-out effect causes cell-cell distances to increase, making it more difficult to group cells into classes based on similarity. 

% barcoding errors
Second, while barcodes enabled increased multiplexing, they are susceptible to errors, which add substantial noise to the sequencing experiments. 
The large number of enzymatic reactions that are necessary to transform the tiny amount of RNA found in the input cells into a cDNA library of adequate concentration to run a sequencer introduce substantial error into the sequences. 
These errors occur in the cell and molecule barcodes of approximately 1\% of reads\footnote{details discussed in~\ref{error_correction}}.
Errors in these barcodes make cells appear to do perplexing things: errors in molecule barcodes make cells appear to express two molecules instead of one, and errors in cell barcodes make cells appear to express molecules that they do not. 
This last characteristic is particularly confounding: 
while drop-out makes us question the meaning of zeros, cell barcode errors make the values we do observe less reliable. 

% ambient contamination
The third and most critical error source in scRNA-seq experiments is ambient contamination.
Cells dislike being dissociated and sorted, two protocols often applied to generate the desired input cell solutions for droplet sequencing. 
As a result of these stressful protocols, some cells will lyse, dumping their mRNA loads into solution. 
...
% todo this paragraph is a bit awkward; doesn't have a coherent purpose
...
Therefore, drop-seq has no guarantee that errors in their will be detectable. 
...
Figure~\ref{fig:indrop-dropseq}~C displays a comparison between Drop-seq and InDrops.
In our hands, InDrop produced data whereing the larger-count cell barcodes were more clearly separable than Drop-seq, and the overall yield of the InDrop experiment was higher.
...
This is an important observation, as it suggests that the overall ambient contamination is lower in the InDrop system, and therefore that the observed molecule counts in InDrop cells have a higher signal to noise ratio. 
...
Therefore, we reasoned that imbalances in GC content could cause some contamination-barcode pairs to amplify well, and cell-barcode pairs to amplify poorly, bluring the boundary between them. 

When we measured the GC contant across cell barcodes, we observed that it was highly variable, with extreme high and low values of 20 and 70\%. 
...
% todo move this section somewhere else or do away with it. 
...
Given this labeled data, one would search for data features that differentiate the two conditions. 
However, the experiments that generate this type labeled data are too easy, and don't generalize well. 
For example, both InDrop and Drop-seq technologies show very low contamination when human and mouse cell lines are mixed, measurable by the number of cells that have both human and mouse DNA\@. 
However, this experiment is both favorable to the assays, since cultered cells don't lyse as frequently as clinical samples, and underpowered, since human and mouse genomes are similar enough that a large number of potentially-contaminating fragments can't be specifically assigned.

As a result, when we attempt to extrapolate what we learn from this case to more complex cases, the features we learned in the simple case vary too much, and fail to clearly differentiate cells in the new experiments. 

These types of control experiments consistently failed to generalize for the hard, high-variance case of sequencing clinical samples.
We theorize this is because we simply have too many free parameters and the biological variation is too high.  % todo make this clearer too. 
As a result, individual algorithms will be evaluated based on their ability to optimize data characteristics, and assumptions, when made, will be stated clearly.
Typically, outcomes will be evaluated in terms of the number of errors that can be resolved or additional molecules that are recovered. 

However, the Chapter will conclude by examining the aggregate impact of the SEQC methods, showing that taken together, these methods were critical to uncovering the biological structure of the data, and that without them, the high inherent noise in scRNA-seq data made it impossible to group cells of similar types or states.  
...
\caption{Not-to-scale schematic of the major components of the GENCODE genome annotation. The sections in red should not be observed by SEQC, either due to beads which remove small molecules, or a lack of poly-a tails, which are used by SEQC to capture RNA.}
...
\section{Barcode Sequencing Errors Arise In Library Construction and Are Correctible} \label{error_correction}
...
The alignments to A, B, C are then uniquely explained by A and can be resolved, whereas the alignments to D, E, and F are joinly explained by E and F, and are ambiguous.}
...
\section{SEQC Indentifies Biological Structure by Removing Noise from scRNA-seq Data}
...
To display the impact of these methods, I compare a single pilot tumor sample analyzed with SEQC with a pipeline constructed from publically available components.
...
Examination of the density projections reveals a single high-density region and two smaller densities (Figure~\ref{fig:seqc-comparison}~A, left).
...
Examining the expression of CD68, a macrophage marker, and CD19 and CD79A, B cell markers, reveals that the locations of B-cells are scattered and disordered. In contrast, SEQC identifies three well circumscribed populations: one macrophage and two B cell (Figure~\ref{fig:seqc-comparison}~B, C). 
More critically, the majority of both B cells and macrophages in the comparison pipeline co-localize. 
This suggests that standard analysis algorithms like PCA and tSNE are unable to differentiate between these very different cell types, and that non-biological signal may be dominating the expression matrices of the comparison pipeline. 

This highlights the importance of rational algorithm design to remove errors from data prior to exploratory data analysis, and highlights the importance of good data processing approaches. Without SEQC, it would not have been possible to analyze patient data. 
...
These samples were profiled by 61 sequencing experiments with at least two technical replicates per sample. 
These sequencing experiments produced over 100,000 cells. 
...
These results suggest that SEQC recovers high quality, low-noise data, and that there is significant variability between our samples which is likely primarily biological. 
The next chapter will discuss how samples from diverse patients, cleaned of technical noise by SEQC, can be compared and integrated in a single, cohesive atlas that can be used to form biological hypotheses about tumor immunology. 
